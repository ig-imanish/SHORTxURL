 ...
 * public void resetCount(HttpSession session, Model model) {
 * countx = 0;
 * count = (Integer) session.getAttribute("count");
 * session.setAttribute("count", countx);
 * session.setAttribute("countx", countx);
 * 
 * model.addAttribute("count", count);
 * }
 * 
 ...
 * @GetMapping("/index")
 * public String showIndexPage(HttpSession session, Model model) {
 * count = (Integer) session.getAttribute("countx");
 * if (count == null) {
 * count = 0;
 * }
 ...
 * 
 * @PostMapping("/shortxurl")
 * public String shortUrlRedirect(@RequestParam("originalUrl") String
 * originalUrl,
 * 
 * @RequestParam("customName") String customName,
 * RedirectAttributes redirectAttributes, HttpSession session, Model model) {
 * if (originalUrl == null || originalUrl.isEmpty() || customName == null ||
 * customName.isEmpty()) {
 * redirectAttributes.addFlashAttribute("urlNotValid",
 * "URL and custom name cannot be empty");
 * return "redirect:/index";
 * }
 * 
 * String regex =
 ...
 * if (originalUrl.length() > 2048 || customName.length() > 50) {
 * redirectAttributes.addFlashAttribute("urlNotValid",
 * "URL or custom name is too long");
 * return "redirect:/index";
 * }
 * 
 * count = (Integer) session.getAttribute("xcount");
 * 
 * System.out.println(count);
 * if (count == null) {
 * count = 0;
 ...
 * 
 * @GetMapping("/{shortUrl}")
 * public ResponseEntity<?> redirectToOrgUrl(@PathVariable("shortUrl") String
 * shortUrl) {
 * Url originalUrl = urlService.getUrlByUrl(shortUrl);
 * if (originalUrl != null) {
 * return
 * ResponseEntity.status(HttpStatus.FOUND).location(URI.create(originalUrl.
 * getOriginalUrl())).build();
 * } else {
 * return ResponseEntity.notFound().build();
 * }
 * }
